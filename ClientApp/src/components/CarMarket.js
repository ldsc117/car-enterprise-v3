...
const filter = (selectComp) => {
    if (selectCompany.length > 0) {
        let temp = carItems
            .filter(item => selectCompany.includes(item.company));
...
useEffect(filter, [selectCompany]);
  ...
  return (
    <div>
      ...
      <h1 className="hidden">{carItems &&
                  carItems.map((element) =>
                    companyList.includes(element.company)
                      ? null
                      : companyList.push(element.company)
                  )}</h1>
        <CardContainer string={string} data={tempCarItems} selectedCompanies={selectCompany}/>
          ...
          <Card>
            ...
            <Accordion.Collapse eventKey="0">
              <Form.Group controlId="formBasicCheckbox">
                ...
                {companyList &&
                  companyList.map((company) => <Form.Check type="checkbox" label={company}
                   id={company}
                   value={company} onChange={handleCheckChange}/>)}
